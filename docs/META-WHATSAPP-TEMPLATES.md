...
#### أ) قالب الدعوة — `content_invitation` (`twilio/call-to-action`)

> لا تستخدم **Card** — واتساب يرفضه أو يسبب 63028. استخدم **Call to action**.  
> زر واحد فقط (فتح الدعوة). الخريطة تظهر داخل صفحة الدعوة.
...
| زر | النوع | القيمة |
|----|-------|--------|
| فتح الدعوة | URL | `https://merhaab.com/i/{{5}}` |

| متغير | المحتوى | عينة |
|-------|---------|------|
| {{1}} | اسم الضيف | `أحمد` |
| {{2}} | اسم المناسبة | `حفل زفاف` |
| {{3}} | التاريخ والوقت | `2026-08-01 - 19:00` |
| {{4}} | المكان | `قاعة الورد الرياض` |
| {{5}} | رمز الضيف (UUID) | `a1b2c3d4-e5f6-7890-abcd-ef1234567890` |

> **بدون** زر خريطة و**بدون** أزرار نعم/لا في هذا القالب.  
> لا تنهِ النص بمتغير — اترك جملة ختامية ثابتة كما فوق.
...
#### ج) تذكير قبل الموعد بيوم — `content_reminder` (`twilio/call-to-action`)
...
نفس متغيرات الدعوة الخمسة. بعد هذه الرسالة يرسل مرحّاب **صورة PNG لرمز QR** تلقائياً.

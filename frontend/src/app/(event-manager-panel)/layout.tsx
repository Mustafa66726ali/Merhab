...
import EventManagerSidebar from "@/components/event-manager-panel/EventManagerSidebar";
import EventManagerHeader from "@/components/event-manager-panel/EventManagerHeader";
...
export default function EventManagerPanelLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (storedUser?.role === "event_manager") {
  ...
  return (
    <div className="flex min-h-screen bg-[#0d0d18] text-on-surface">
      <EventManagerSidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
